...
public class NearestNeighborVisualizer {
    ...
    public static void main(String[] args) {
    	...
    	String filename = "src/s3/circle10000.txt";
        ...
        double x, y;
        for(int i = 0 ; i < n; i++) {
        	StdOut.println("INSERTING: " + i);
        	x = StdRandom.uniform(0.0, 1.0);
        	y = StdRandom.uniform(0.0, 1.0);
        	Point2D testPoint= new Point2D(x, y);
        	list[i] = testPoint;
        }
        StdOut.println("START CLOCK");
        for(int i = 0; i < n; i++) {
        	StdOut.println("NowChecking: " + i);
        	kdtree.insert(list[i]);
        }
        ...
        for(int i = 0 ; i < 100; i++) {
        	x1 = StdRandom.uniform(0.0, 1.0);
        	y1 = StdRandom.uniform(0.0, 1.0);
        	Point2D testPoint1= new Point2D(x1, y1);
        	kdtree.nearest(testPoint1);
        }
